...
def smooth_labels(labels, num_classes, smoothing=0.1):
    ...
    return one_hot * confidence + (1 - one_hot) * low_conf
...
def get_cnn_model(n_in, n_hidden, n_out, dropout_p=0.5):
    net = dict()
    net['input'] = lasagne.layers.InputLayer(shape=(None, n_in[1], n_in[2], n_in[3]))
    
    #### ADD DROPOUT TO INPUT ####
    net['dropout_input'] = lasagne.layers.DropoutLayer(net['input'], p=dropout_p)

    net['conv1'] = lasagne.layers.Conv2DLayer(net['dropout_input'], num_filters=32, filter_size=(5, 5),
            pad='same',
    ...
    net['dropout_fc'] = lasagne.layers.DropoutLayer(net['fc'], p=dropout_p)
...
def get_nn_model(n_in, n_hidden, n_out, dropout_p=0.5):
    net = dict()
    print(n_in)
...
def train_model(dataset, n_hidden=50, batch_size=100, epochs=100, learning_rate=0.01, model='cnn', l2_ratio=1e-7):
    ...
    target_var = T.matrix('y') # smoothed one-hot labels!
    if model == 'cnn':
